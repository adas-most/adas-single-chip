		...
		try
		{
            ...
            if(Request != null)
            {
	            ...
	            if(Request.getString("method").equals("GET"))
	            {
	    			...
	    			{
	    				if(request_URL.split("\\?")[1].equals("1") ||
	    				   request_URL.split("\\?")[1].equals("2"))
    					{
	    			...
	    			if(request_URL.equals("/video"))
	    			{
	    				if(Start.eventid == 1)
	    				{
	    					videoName = "sample_fix.mp4";
	    				}
	    				if(Start.eventid == 2)
	    				{
	    					videoName = "sample_fix_no_rain.mp4";
	    				}
	    				
	    				TCPSC.getFile(videoName);
...
//	            	TODO 選擇影像處理模組
	            	if(request_URL.equals("/IPM_confirmUseModules"))
	            	{
	            		...
	            		ResponseHeaders = "HTTP/1.0 200 OK\r\n"
	            				 ...
	            				 + "Content-Type: application/json; charset=utf-8\r\n"
	            				 + "\r\n" ;
	            		ConfirmUseModules modifyModulesParameter = new ConfirmUseModules();
	            		
	            		JSONObject ResponseMassage = new JSONObject();
	            		
	            		ResponseMassage = modifyModulesParameter.updateConfirmUseModules(Request.getJSONObject("RequestMassage"));
	...
	public boolean HttpFilePacketMaker(String Message) 
	{
		...
		{			
            ...
            if(this.ResponseFile.exists())
            {
				...
				if(this.logFlag) System.out.println("取得檔案 ： " + this.Filelocal + Message);
